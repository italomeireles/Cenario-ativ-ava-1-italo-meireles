...
const pescoco = new Mesh(
  ...
  new MeshToonMaterial({
    color: new Color(PARAMS.pescoco),  
    wireframe: false,
  })
)
...
predio_1.position.set(-18, 6.6, -15)
...
const braco_esq = new Mesh(
  ...
  new MeshToonMaterial({
    color: new Color(PARAMS.braco_esq),  
    wireframe: false,
...
const braco_dir = new Mesh(
  ...
  new BoxGeometry(0.5, 1.7, 0.5),
  new MeshToonMaterial({
    color: new Color(PARAMS.braco_dir),  
    wireframe: false,
...
sphereCtrls.addInput(solo.position, "x", {
  label: "pos x",
  min: -10,
  max: 10,
  step: 0.1,
})
sphereCtrls.addInput(solo.position, "y", {
  label: "pos y",
  min: -10,
  max: 10,
  step: 0.1,
})
sphereCtrls.addInput(solo.position, "z", {
  label: "pos z",
  min: -10,
  max: 10,
  step: 0.1,
...
sphereCtrls.addInput(PARAMS, "color").on("change", (e) => {
  solo.material.color = new Color(e.value)
})

sphereCtrls.addInput(solo.material, "wireframe")
*/
scene.add(predio_1)
